...
class ImageTestCase(TestCase):
    def setUp(self):
        test_image_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'test_images', 'test.jpg')
        ...
        self.user_plan2 = UserPlan.objects.create(user=self.user2,plan=self.premium_plan)
        
        self.expiry_image_1 = Image.objects.create(user=self.user,image=test_image_path,expiry_seconds=2000)
...
class ImageAPITestCase(APITestCase):
    def setUp(self):
        self.test_image_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'test_images', 'test.jpg')
        self.user = User.objects.create_superuser(
            username='admin123',
            email='admin123@admin123.com',
            password='fooooobar')
